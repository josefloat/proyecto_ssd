...
- Publicar y reservar únicamente slots cuyo `inicioUtc` sea estrictamente posterior al reloj de la operación; un slot pasado o que inicia exactamente ahora no admite reserva ni escrituras parciales.
...
- Expirar una cita `RESERVADA` hasta 72 horas después de crearla y nunca después del inicio del slot, registrándola como `CANCELADA` por `EXPIRACION` y liberando el slot atómicamente antes de operaciones que dependan de disponibilidad o estado.
...
- `disponibilidad-publica-api`: La disponibilidad pública excluye slots pasados o que empiezan exactamente ahora y conserva únicamente slots futuros.
...
- **Backend:** nuevos servicios transaccionales y rutas públicas de reserva, consulta y cancelación; la disponibilidad aplicará expiraciones vencidas y filtrará por `inicioUtc > ahora` antes de publicar slots libres.
