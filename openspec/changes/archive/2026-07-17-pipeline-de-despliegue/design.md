...
Verificación del escenario "Violación de accesibilidad en preview bloquea la promoción": se probó en una rama descartable (primero `fixture/axe-blocks-promotion`; repetida como `fixture/axe-blocks-promotion-v2` tras la Decisión 11, porque cambiaron los nombres de los pasos posteriores a la promoción; ambas borradas tras la prueba, nunca mergeadas) con un `<img>` sin `alt` añadido a propósito a la home y los guards de rama de `migrate`/`deploy-vercel` relajados temporalmente para esa rama (el guard de `deploy-render` se dejó intacto para no tocar Render). La corrida real ([run 29555671389](https://github.com/josefloat/proyecto_ssd/actions/runs/29555671389)) confirmó, vía la API de GitHub Actions, que el paso de axe-core terminó en `failure` y que "Promover el preview a producción", "Esperar al deployment de producción real y confirmar su commit" y "Smoke HTTP contra el dominio real de producción" quedaron en `skipped` — la promoción nunca se ejecutó, `deploy-render` nunca corrió.
...
Verificación del camino feliz en producción real ([run 29555404046](https://github.com/josefloat/proyecto_ssd/actions/runs/29555404046)): `targets.production.readyState` transicionó `BUILDING` → `READY` (confirmando empíricamente que promover sí reconstruye, no solo realiasa); `meta.commitSha` sobrevivió el rebuild y coincidió exactamente con `github.sha` (`955d69ee34820880f3e26be4450689158e388c16`); el dominio real resuelto fue `senal-de-vida-frontend.vercel.app`, con smoke HTTP `200`.

**12. Smokes reales post-deploy en un job separado (`runtime-smoke`) que espera a ambos proveedores.**
Defecto encontrado en la primera versión de las tareas 4.1-4.2: `deploy-render` y `deploy-vercel` corren en paralelo (ambos dependen solo de `migrate`), y el smoke del recorrido completo por el proxy vivía dentro de `deploy-vercel`. Como el proxy reenvía a Render, ese smoke podía ejecutarse y responder `200` contra el backend *anterior* mientras `deploy-render` todavía estaba desplegando el nuevo `github.sha` — un falso verde que no probaría el recorrido completo contra el commit correcto en ambos extremos. Corrección: se extrae un job `runtime-smoke` con `needs: [deploy-render, deploy-vercel]`, gateado a `main` igual que el resto del despliegue; solo arranca cuando ambos deploys confirmaron su propio commit live. `deploy-render` expone `backend_url` (resuelto vía `GET /v1/services/{id}` de la API de Render) y `deploy-vercel` expone `frontend_domain` como outputs de job para que `runtime-smoke` los consuma. `deploy-vercel` conserva el smoke ligero de `/` que pertenece a la verificación de producción de la Decisión 11; solo los dos smokes de runtime (`/health` directo y el recorrido vía proxy) se movieron.
Verificación en producción real ([run 29559356102](https://github.com/josefloat/proyecto_ssd/actions/runs/29559356102), commit `c8f729b6f61e1eec0b0904cdb7dda52df15b6440`): `deploy-render` confirmó el commit live a las 06:14:31Z ("Render desplegó exactamente github.sha y está live."); `deploy-vercel` confirmó `targets.production.meta.commitSha` contra el mismo commit a las 06:15:40Z (el más tardío de los dos, dominio `senal-de-vida-frontend.vercel.app`); `runtime-smoke` arrancó a las 06:15:44Z, después de que ambos terminaran. `smoke-health.sh` contra `https://senal-de-vida-api.onrender.com` respondió `200` con `db:"ok"` en el intento 1 (t=0s de una ventana de 90s); `smoke-proxy-health.sh` contra `https://senal-de-vida-frontend.vercel.app` respondió `200` con `db:"ok"` en el intento 1 (t=1s de una ventana de 120s). Los cinco jobs del pipeline (`build-and-test`, `migrate`, `deploy-render`, `deploy-vercel`, `runtime-smoke`) terminaron en verde.

**13. Verificación final del sprint (5.1): pipeline completo en verde en producción real.**
Cierra el sprint confirmando, en una sola corrida real en `main`, todo lo que las decisiones 1-12 diseñaron: build/pruebas, migración, despliegue SHA-pinned en ambos proveedores, gate de accesibilidad en preview, y los tres smokes de runtime (`/live`, `/health`, recorrido vía proxy) contra la infraestructura real.
Verificación en producción real ([run 29560274248](https://github.com/josefloat/proyecto_ssd/actions/runs/29560274248), commit `2d4d85d86baf8d3f3c37848dc8e883ccdbd554a2`):
- `build-and-test` (2m4s): verde, incluidos los 4 archivos/7 tests de Vitest — `tests/live.test.ts` (2 tests) confirma que `/live` nunca invoca `$queryRaw`, ni siquiera con la base de datos inalcanzable.
- `migrate` (17s): verde contra Neon (`DIRECT_URL`).
- `deploy-render` (38s) y `deploy-vercel` (1m53s) confirmaron, cada uno por su cuenta, exactamente el mismo commit `2d4d85d86baf8d3f3c37848dc8e883ccdbd554a2` ("Commit desplegado coincide con el esperado").
- `axe-core` contra el preview: `1 passed (3.3s)` — cero violaciones de accesibilidad detectadas antes de promover.
- Smoke HTTP contra el dominio real de producción de Vercel (`senal-de-vida-frontend.vercel.app`): `status=200`.
- `runtime-smoke` (6s) arrancó a las 06:37:29Z, después de que `deploy-render` (completó 06:36:12Z) y `deploy-vercel` (completó 06:37:27Z, el más tardío) terminaran — confirmando que `needs: [deploy-render, deploy-vercel]` (Decisión 12) efectivamente bloquea hasta que ambos despliegues están live.
- `/live` real (`https://senal-de-vida-api.onrender.com/live`): `status=200 body={"status":"ok"}`.
- `/health` real (mismo dominio): `status=200 body={"status":"ok","db":"ok"}`, intento 1, t=0s de una ventana de 90s.
- `/api/health` real a través del proxy (`https://senal-de-vida-frontend.vercel.app`): `200` con `db:"ok"`, intento 1, t=0s de una ventana de 120s.

Los 5 jobs del pipeline (`build-and-test`, `migrate`, `deploy-render`, `deploy-vercel`, `runtime-smoke`) terminaron en verde en esta corrida.
